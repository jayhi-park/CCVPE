...
from networks.VGG2 import VGG3, VGG3_Cycle
...
class CVM_VIGOR(nn.Module):
    ...
    def forward(self, grd, sat):
        grd_feature_volume = self.grd_efficientnet.extract_features(grd) # [B, 1280, 10, 20]
        grd_descriptor1 = self.grd_feature_to_descriptor1(grd_feature_volume) # length 1280 # [B, 1280]
        grd_descriptor2 = self.grd_feature_to_descriptor2(grd_feature_volume) # length 640 # [B, 640]
        grd_descriptor3 = self.grd_feature_to_descriptor3(grd_feature_volume) # length 320 # [B, 320]
        grd_descriptor4 = self.grd_feature_to_descriptor4(grd_feature_volume) # length 160 # [B, 160]
        grd_descriptor5 = self.grd_feature_to_descriptor5(grd_feature_volume) # length 80 # [B, 80]
        grd_descriptor6 = self.grd_feature_to_descriptor6(grd_feature_volume) # length 40 # [B, 40]
        
        grd_descriptor_map1 = grd_descriptor1.unsqueeze(2).unsqueeze(3).repeat(1, 1, 8, 8) # [1, 1280, 8, 8]
        grd_descriptor_map2 = grd_descriptor2.unsqueeze(2).unsqueeze(3).repeat(1, 1, 16, 16) # [1, 640, 16, 16]
        grd_descriptor_map3 = grd_descriptor3.unsqueeze(2).unsqueeze(3).repeat(1, 1, 32, 32) # [1, 320, 32, 32]
        grd_descriptor_map4 = grd_descriptor4.unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64) # [1, 160, 64, 64]
        grd_descriptor_map5 = grd_descriptor5.unsqueeze(2).unsqueeze(3).repeat(1, 1, 128, 128) # [1, 80, 128, 128]
        grd_descriptor_map6 = grd_descriptor6.unsqueeze(2).unsqueeze(3).repeat(1, 1, 256, 256) # [1, 40, 256, 256]

        # sat_feature_volume: [1, 1280, 16, 16]
        ...
        # matching bottleneck
        grd_des_len = grd_descriptor1.size()[1] # [B, 1280]
        sat_des_len = sat_descriptor_map.size()[1] # [B, 1280, 8, 8]
        grd_map_norm = torch.norm(grd_descriptor_map1, p='fro', dim=1, keepdim=True) # [B, 1280, 8, 8]
...
class CVM_VIGOR_VGG(nn.Module):
    def __init__(self, device, circular_padding):
        super(CVM_VIGOR_VGG, self).__init__()
        self.device = device
        self.circular_padding = circular_padding

        self.SatFeatureNet = VGG3(None, 6)
        self.GrdFeatureNet = VGG3_Cycle(None, -1)

        # self.SatFeatureNet = VGGUnet3_Cycle(self.args, self.level)
        ...
        # loc
        self.deconv6 = nn.ConvTranspose2d(1281, 1024, 2, 2)
        self.conv6 = double_conv(1536, 640)

        self.deconv5 = nn.ConvTranspose2d(641, 320, 2, 2)
        self.conv5 = double_conv(832, 320)

        self.deconv4 = nn.ConvTranspose2d(321, 160, 2, 2)
        self.conv4 = double_conv(416, 160)

        self.deconv3 = nn.ConvTranspose2d(161, 80, 2, 2)
        self.conv3 = double_conv(208, 80)

        self.deconv2 = nn.ConvTranspose2d(81, 40, 2, 2)
        self.conv2 = double_conv(104, 40)
        ...
        # ori
        self.deconv6_ori = nn.ConvTranspose2d(1300, 1024, 2, 2)
        self.conv6_ori = double_conv(1536, 640)

        self.deconv5_ori = nn.ConvTranspose2d(640, 256, 2, 2)
        self.conv5_ori = double_conv(768, 256)

        self.deconv4_ori = nn.ConvTranspose2d(256, 128, 2, 2)
        self.conv4_ori = double_conv(384, 128)

        self.deconv3_ori = nn.ConvTranspose2d(128, 64, 2, 2)
        self.conv3_ori = double_conv(192, 64)

        self.deconv2_ori = nn.ConvTranspose2d(64, 32, 2, 2)
        self.conv2_ori = double_conv(96, 32)
    ...
    def forward(self, grd, sat):
        grd_feature_volume = self.GrdFeatureNet(grd)
        ...
        grd_descriptor_map1 = grd_descriptor1.unsqueeze(2).unsqueeze(3).repeat(1, 1, 8, 8) # [B, 1280, 8, 8]
        grd_descriptor_map2 = grd_descriptor2.unsqueeze(2).unsqueeze(3).repeat(1, 1, 16, 16) # [B, 640, 16, 16]
        grd_descriptor_map3 = grd_descriptor3.unsqueeze(2).unsqueeze(3).repeat(1, 1, 32, 32) # [B, 320, 32, 32]
        grd_descriptor_map4 = grd_descriptor4.unsqueeze(2).unsqueeze(3).repeat(1, 1, 64, 64) # [B, 160, 64, 64]
        grd_descriptor_map5 = grd_descriptor5.unsqueeze(2).unsqueeze(3).repeat(1, 1, 128, 128) # [B, 80, 128, 128]
        grd_descriptor_map6 = grd_descriptor6.unsqueeze(2).unsqueeze(3).repeat(1, 1, 256, 256) # [B, 40, 256, 256]

        # sat_feature_volume: [1, 1024, 16, 16]
        sat_feature_volume, multiscale_sat = self.SatFeatureNet(sat)
        sat_feature_block0 = multiscale_sat[4]  # [32, 256, 256]
        sat_feature_block2 = multiscale_sat[3]  # [64, 128, 128]
        sat_feature_block4 = multiscale_sat[2]  # [128, 64, 64]
        sat_feature_block10 = multiscale_sat[1]  # [256, 32, 32]
        sat_feature_block15 = multiscale_sat[0]  # [512, 16, 16]
        ...
        for i in range(20):
            sat_descriptor_map_rolled = torch.roll(sat_descriptor_map, shifts=-i * 64, dims=1) # [B, 1024, 8, 8]
            sat_descriptor_map_window = sat_descriptor_map_rolled[:, :grd_des_len, :, :] # [B, 1024, 8, 8]
            sat_map_norm = torch.norm(sat_descriptor_map_window, p='fro', dim=1, keepdim=True) # [B, 1, 8, 8]
